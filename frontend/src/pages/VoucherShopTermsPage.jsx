...
export default function VoucherShopTermsPage() {
  ...
  const title = page.brand_title || config.branding?.firma_nazev || config.branding?.app_title || 'Dárkové poukazy';
  const termsTitle = page.terms_title || 'Obchodní podmínky';
    ...
    <div className="min-h-screen bg-stone-50">
      <header className="border-b border-stone-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3">
            {shopLogo ? (
              <img src={shopLogo} alt={title} className="h-10 w-10 object-contain" />
            ) : (
            ...
            <div>
              ...
              <div className="text-xs text-stone-500">{termsTitle}</div>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-3xl px-5 py-8">
        <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          <h1 className="text-2xl font-bold text-stone-900">{termsTitle}</h1>
