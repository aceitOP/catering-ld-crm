            ...
            <div>
              {posledni.map((z, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-stone-50 transition-colors ${i < posledni.length - 1 ? 'border-b border-stone-50' : ''}`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-stone-900 truncate">{z.nazev}</div>
                    <div className="text-xs text-stone-400 mt-0.5">{z.cislo} · {formatDatum(z.datum_akce)}</div>
                  </div>
                  <TypBadge typ={z.typ} />
                  <StavBadge stav={z.stav} />
                  <div className="text-sm font-medium text-stone-700 text-right min-w-[90px]">
                    {formatCena(z.cena_celkem)}
                  </div>
                </div>
              ))}
              ...
              {upcoming.map(e => {
                ...
                return (
                  <div
                    ...
                    className="flex gap-3 px-4 py-3 hover:bg-stone-50 cursor-pointer transition-colors"
                  >
                    <div className="bg-stone-100 rounded-lg px-2 py-1.5 text-center min-w-[40px]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-stone-400 mt-0.5 truncate">
                        {e.klient_jmeno} {e.klient_prijmeni}
                      </div>
                      <TypBadge typ={e.typ} />
                    </div>
                  </div>
