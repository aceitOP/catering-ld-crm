...
export default function EmailPage() {
  ...
  const zakazkaMut = useMutation({
    mutationFn: ({ uid }) => emailApi.createZakazka(uid, folder),
    onSuccess: (res) => {
      toast.success(`Zakázka ${res.data?.cislo} vytvořena`);
      navigate(`/zakazky/${res.data?.zakazka_id}`);
    },
    onError: (e) => toast.error(e?.response?.data?.error || 'Nepodařilo se vytvořit zakázku'),
  });
        ...
        {!selUid ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2 px-5 py-3 border-b border-stone-100 flex-shrink-0">
              ...
              <button
                onClick={() => zakazkaMut.mutate({ uid: selUid })}
                disabled={zakazkaMut.isPending}
              ...
              >
                {zakazkaMut.isPending ? <Loader2 size={13} className="animate-spin" /> : <ClipboardList size={13} />}
                Vytvořit zakázku
              </button>
