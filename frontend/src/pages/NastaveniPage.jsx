        ...
        <div className="border-t border-stone-100 pt-3 space-y-2">
          ...
          <div><label className="text-xs text-stone-400 block mb-0.5">Text šablony</label>
            <textarea rows={4} className="w-full border border-stone-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none resize-none"
              placeholder="Vážený zákazníku, potvrzujeme Váš termín..."
...
export function NastaveniPage() {
  ...
  const handleLogoChange = async (event) => {
    ...
    if (!['image/png', 'image/jpeg', 'image/webp', 'image/svg+xml'].includes(file.type)) {
      toast.error('Logo musí být PNG, JPG, SVG nebo WEBP');
      return;
    }
    if (file.size > 512 * 1024) {
      toast.error('Logo může mít maximálně 512 KB');
      return;
    }
              ...
              <div className="rounded-xl border border-stone-200 p-4 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="text-xs text-stone-500 mt-1">Použije se v loginu i v hlavičce aplikace. Když logo nevložíte, zobrazí se text Catering CRM.</div>
                  </div>
