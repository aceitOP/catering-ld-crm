...
function formatBackupStatus(status) {
  if (status === 'success') return { label: 'V poradku', className: 'text-emerald-700' };
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-3">
      ...
      {sablony.length === 0 && !editing && (
        <div className="text-xs text-stone-400 py-3 text-center">Ĺ˝ĂˇdnĂ© Ĺˇablony â€” kliknÄ›te PĹ™idat</div>
      )}
      ...
      {sablony.map(s => (
        <div key={s.id} className="flex items-start justify-between gap-3 py-2 border-t border-stone-100">
          <div className="flex-1 min-w-0">
            ...
            {s.predmet_prefix && <div className="text-xs text-stone-400">PĹ™edmÄ›t: {s.predmet_prefix}</div>}
            <div className="text-xs text-stone-500 truncate mt-0.5">{s.telo?.slice(0, 80)}{s.telo?.length > 80 ? 'â€¦' : ''}</div>
          </div>
        ...
        <div className="border-t border-stone-100 pt-3 space-y-2">
          ...
          <div><label className="text-xs text-stone-400 block mb-0.5">Text Ĺˇablony</label>
            <textarea rows={4} className="w-full border border-stone-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none resize-none"
              placeholder="VĂˇĹľenĂ­, potvrzujeme VĂˇĹˇ termĂ­nâ€¦"
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Btn size="sm" variant="primary" onClick={save}
              ...
              {createMut.isPending || updateMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit Ĺˇablonu'}
            </Btn>
          </div>
        </div>
      )}
...
function SmtpTestButton() {
  ...
  return (
    <div className="pt-2 border-t border-stone-100 space-y-2">
      ...
      >
        {testing ? 'Testuji spojenĂ­â€¦' : 'Otestovat SMTP pĹ™ipojenĂ­'}
      </button>
      {result && (
        <div className={`rounded-lg p-3 text-xs space-y-1 ${result.ok ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-red-50 text-red-800 border border-red-100'}`}>
          <div className="font-semibold">{result.ok ? 'âś“ PĹ™ipojenĂ­ ĂşspÄ›ĹˇnĂ©' : 'âś— PĹ™ipojenĂ­ selhalo'}</div>
          {result.info && <div className="text-stone-500">{result.info.host}:{result.info.port} Â· {result.info.secure ? 'SSL' : 'STARTTLS'} Â· {result.info.user}</div>}
...
export function NastaveniPage() {
  ...
  const saveMut   = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.error || 'Nastaveni se nepodarilo ulozit'),
  });
  const userMut   = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.error || 'Uzivatele se nepodarilo pridat'),
  });
  const toggleMut = useMutation({
    mutationFn: ({id,aktivni}) => uzivateleApi.update(id,{aktivni}),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['uzivatele'] });
      toast.success('Stav uzivatele byl upraven');
    },
    onError: (err) => toast.error(err?.response?.data?.error || 'Stav uzivatele se nepodarilo zmenit'),
  });
  const deleteMut = useMutation({
  ...
  return (
    <div>
      ...
      <div className="p-6 max-w-2xl">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
              <div className="flex items-center justify-between pb-3 mb-1 border-b border-stone-100">
                ...
                <Btn size="sm" onClick={() => { qc.clear(); qc.invalidateQueries(); toast.success('Cache vymazĂˇna, data se obnovujĂ­â€¦'); }}>Vymazat cache</Btn>
              </div>
              ...
              <div className="flex justify-end">
                <Btn variant="primary" onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>
                  {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit zmÄ›ny'}
                </Btn>
              </div>
            </div>
          </div>
        ...
        {tab === 'moduly' && isSuperAdmin && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
              ...
              <div className="flex justify-end">
                <Btn variant="primary" onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>
                  {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit moduly'}
                </Btn>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
            ...
            <div className="flex justify-end pt-1">
              <Btn variant="primary"
                ...
                {passMut.isPending ? 'MÄ›nĂ­mâ€¦' : 'ZmÄ›nit heslo'}
              </Btn>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Btn variant="primary" onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>
                  {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit podpis'}
                </Btn>
              </div>
              {podpisPreview && (
        ...
        {tab === 'notif' && (
          <div className="bg-white rounded-xl border border-stone-200 p-5">
            <p className="text-sm text-stone-500">NastavenĂ­ notifikacĂ­ bude dostupnĂ© po propojenĂ­ s e-mailovĂ˝m systĂ©mem.</p>
          </div>
        )}
              ...
              <div className="flex items-start justify-between">
                ...
                {gcStatus && (
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full flex-shrink-0 ml-4 ${gcStatus.connected ? 'bg-green-50 text-green-700' : 'bg-stone-100 text-stone-500'}`}>
                    {gcStatus.connected ? 'âś“ PĹ™ipojeno' : 'NepĹ™ipojeno'}
                  </span>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Btn variant="primary" onClick={() => { saveMut.mutate(form); setTimeout(() => refetchGcStatus(), 1000); }} disabled={saveMut.isPending}>
                    {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit'}
                  </Btn>
                </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-stone-100">
                <Btn variant="primary" onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>
                  {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit limity'}
                </Btn>
        ...
        {tab === 'email' && nastavData && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-stone-100">
                <Btn variant="primary" onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>
                  {saveMut.isPending ? 'UklĂˇdĂˇmâ€¦' : 'UloĹľit nastavenĂ­'}
                </Btn>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
        ...
        {tab === 'login-log' && isSuperAdmin && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
              {loginLogLoading ? (
                <div className="p-8 text-center text-sm text-stone-400">NaÄŤĂ­tĂˇmâ€¦</div>
        ...
        {tab === 'zaloha' && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
              ...
              {backupInfoLoading ? (
                <div className="text-xs text-stone-400">NaÄŤĂ­tĂˇm pĹ™ehledâ€¦</div>
              ) : backupInfo?.counts ? (
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-stone-100">
                ...
                <div className="rounded-lg bg-stone-50 border border-stone-200 px-3 py-2 text-xs text-stone-500 space-y-1">
                  <div>Posledni beh: <span className="font-medium text-stone-700">{backupInfo?.settings?.lastRunAt ? new Date(backupInfo.settings.lastRunAt).toLocaleString('cs-CZ') : 'â€”'}</span></div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-stone-100">
                ...
                <Btn variant="primary" onClick={handleDownloadBackup} disabled={backupLoading}>
                  <Download size={12}/>
                  {backupLoading ? 'PĹ™ipravuji zĂˇlohuâ€¦' : 'StĂˇhnout zĂˇlohu (JSON)'}
                </Btn>
              </div>
            </div>
            ...
            {!backupInfoLoading && backupInfo && !backupInfo.files?.length && (
              <div className="bg-white rounded-xl border border-dashed border-stone-200 p-5 text-sm text-stone-500">
                Zatim tu nejsou ulozene zadne serverove zalohy. Vytvorte prvni rucne nebo pockejte na auto-backup.
              </div>
            )}
      ...
      <Modal open={userModal} onClose={() => setUserModal(false)} title="NovĂ˝ uĹľivatel"
        footer={<><Btn onClick={() => setUserModal(false)}>ZruĹˇit</Btn><Btn variant="primary" onClick={() => userMut.mutate(userForm)} disabled={!userForm.jmeno||!userForm.email||userMut.isPending}>{userMut.isPending?'UklĂˇdĂˇmâ€¦':'PĹ™idat'}</Btn></>}>
