...
const DEFAULT_NOTIFICATION_RULES = [
  {
    ...
    title: 'Zakazka potvrzena',
    description: 'Posle interni notifikaci a volitelne e-mail pri potvrzeni zakazky.',
    event_type: 'zakazka_confirmed',
    enabled: true,
    include_assigned_staff: false,
    include_admins: true,
    extra_emails: '',
    subject_template: 'Potvrzena zakazka: {cislo} - {nazev}',
    body_template: 'Zakazka {cislo} ({nazev}) byla potvrzena. Termin: {datum_akce}. Misto: {misto}.',
  },
  {
    key: 'zakazka_changed',
    title: 'Zmena terminu nebo venue',
    description: 'Upozorni na zmenu dulezitych detailu eventu.',
    event_type: 'zakazka_changed',
    enabled: true,
    include_assigned_staff: true,
    include_admins: true,
    extra_emails: '',
    subject_template: 'Zmena zakazky: {cislo} - {nazev}',
    body_template: 'Zakazka {cislo} ({nazev}) ma zmenene dulezite detaily. Termin: {datum_akce}. Misto: {misto}.',
  },
  {
    key: 'upcoming_event_48h',
    title: 'Akce za 48 hodin',
    description: 'Pripomene blizici se akci internimu tymu a pripadne prirazenemu personalu.',
    event_type: 'upcoming_event_48h',
    enabled: true,
    include_assigned_staff: true,
    include_admins: true,
    extra_emails: '',
    subject_template: 'Blizi se akce: {cislo} - {nazev}',
    body_template: 'Akce {cislo} ({nazev}) probiha {datum_akce} v {cas_zacatek}. Misto: {misto}.',
  },
  {
    key: 'missing_venue_debrief',
    title: 'Chybi venue debrief',
    description: 'Upozorni po realizaci, ze k venue jeste chybi debrief.',
    event_type: 'missing_venue_debrief',
    enabled: true,
    include_assigned_staff: false,
    include_admins: true,
    extra_emails: '',
    subject_template: 'Chybi venue debrief: {cislo} - {nazev}',
    body_template: 'Zakazka {cislo} ({nazev}) je realizovana, ale venue debrief zatim chybi.',
  },
  {
    key: 'venue_debrief_submitted',
    title: 'Venue debrief vyplnen',
    description: 'Informuje o pridanem venue debriefu a novych pozorovanich.',
    event_type: 'venue_debrief_submitted',
    enabled: true,
    include_assigned_staff: false,
    include_admins: true,
    extra_emails: '',
    subject_template: 'Venue debrief vyplnen: {cislo} - {nazev}',
    body_template: 'Zakazka {cislo} ({nazev}) ma novy venue debrief. Zkontrolujte observations a pripadne promotnuti do venue master dat.',
  },
];
...
async function runScheduledNotificationSweep(dbQuery = query) {
  ...
  for (const zakazka of missingDebriefs.rows) {
    await processNotificationEvent({
      ...
      title: `Chybi venue debrief: ${zakazka.cislo} - ${zakazka.nazev}`,
